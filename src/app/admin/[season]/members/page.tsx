...
export default async function Members({ params }: { params: Promise<{ season: string }> }) {
  ...
  return (
    <AdminShell season={season} active="/members">
      <PageHeader eyebrow="Commissioner" title="Members">
              ...
              <Card key={t.id} className="p-4">
                <p className="flex items-center justify-between gap-2">
                  ...
                  <span className="flex flex-col items-end gap-1">
                    <StatusBadge tone={occupant ? "good" : "neutral"}>{occupant ? occupant.username : "Unassigned"}</StatusBadge>
                    {commissioners.has(t.id) ? <StatusBadge tone="accent">Commissioner</StatusBadge> : null}
                    {season.wagerState !== "OFF" ? <StatusBadge tone={wagered.has(t.id) ? "good" : "neutral"}>{wagered.has(t.id) ? "Wager placed" : "No wager yet"}</StatusBadge> : null}
                  </span>
                </p>
